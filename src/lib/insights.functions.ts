...
export const generateInsights = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<CourseInsight[]> => {
    ...
    const { buildInsightsRequest, parseInsights, streamGatewayJson } = await import("@/lib/insights.server");

    const raw = await streamGatewayJson(buildInsightsRequest(data.facts as never), apiKey);
